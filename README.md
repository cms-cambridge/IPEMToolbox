...
| **`tests/`** | Headless Octave smoke test, Leman-2000 pipeline check, and systematic `resample` suite |
...
`tests/test_resample.m` covers the shim in four layers:

1. **Filter design** — `firls` + `kaiser(N=10, β=5)` matches the analytic windowed-sinc formula; returned `h` has the MATLAB-style leading zero pad
2. **Polyphase properties** — output length, orientation, column-wise matrices, impulse peak location, and agreement with Octave Forge when given the same `h`
3. **Independent fixtures** — compares against `tests/fixtures/resample/` generated by `tests/generate_resample_fixtures.py` (SciPy reimplementation of the same algorithm; regenerate with `python3 tests/generate_resample_fixtures.py`)
4. **16-bit contract** — documents that a one-LSB float tweak at PCM scale can change quantized samples (the sensitivity path inside `IPEMCalcANI`)

```bash
octave --no-gui --eval "addpath('tests'); assert(test_resample()==0)"
```
