...
The steps to compile and use the toolbox in GNU Octave are very similar. Make sure the @mkoctfile@ command is available in your path before running make. On Debian/Ubuntu this is found in the @octave-dev@ package (older releases used @octave-pkg-dev@). You also need the Octave @signal@ package (e.g. @apt install octave-signal@, or @pkg install -forge signal@).

h3. GNU Octave compatibility

This fork includes @IPEMToolbox/OctaveCompat@ helpers for running under modern GNU Octave (tested with 10.3):

* @wavread@ / @wavwrite@ / @wavplay@ / @auread@ / @auwrite@ shims over @audioread@ / @audiowrite@
* a MATLAB-compatible @resample@ (Octave's default anti-aliasing filter differs and shifts Leman-2000 correlations by ~1.5e-4)
* Octave-aware @IPEMSetup@ (loads the @signal@ package; skips MATLAB-only path persistence)

After compiling the auditory model under @AuditoryModel/Octave_UNIX@ and running @IPEMSetup@, headless checks live in @tests/@:

bc. octave --no-gui --eval "run('tests/smoke_test_octave.m')"

Container packaging for the Leman (2000) pipeline lives in "pyLeman2000":https://github.com/cms-cambridge/pyLeman2000, which builds a Docker image from this toolbox. The GUIDE-based @IPEMMECReSynthUI@ is not supported under Octave.
