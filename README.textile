...
h3. Docker (GNU Octave 10.3)

A Dockerfile is provided for license-free containerized use via GNU Octave 10.3. It compiles the auditory model MEX inside the image and installs Octave Forge @signal@ (plus dependencies).

bc. docker build -t ipem-toolbox:octave-10.3 .
docker run --rm ipem-toolbox:octave-10.3

The default command runs @tests/smoke_test_octave.m@. For an interactive Octave session:

bc. docker run --rm -it ipem-toolbox:octave-10.3 octave --no-gui

Compatibility helpers live in @IPEMToolbox/OctaveCompat@ (@wavread@/@wavwrite@/@wavplay@/@textread@ shims). The GUIDE-based @IPEMMECReSynthUI@ is not supported under Octave.
